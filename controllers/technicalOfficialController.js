...
const shouldExposeLoginHistory = (req) =>
  Boolean(req && req.admin && req.admin.role === "superadmin");

const buildRegistrationNumber = (official) => {
  if (!official) return "";
  if (official.registrationNumber) return official.registrationNumber;
  return `DDKA-2026-${String(official._id || "")
    .slice(-4)
    .toUpperCase()}`;
};
...
const buildPublicTemplateUrl = (req, official, assetType) => {
  ...
  if (assetType === "id-card") {
    ...
    if (official.dob) {
      params.set("dob", new Date(official.dob).toISOString().slice(0, 10));
    }
...
const deleteCloudinaryByUrl = async (url) => {
  ...
  try {
    ...
    console.error(
      "Failed to delete Cloudinary asset for technical official:",
      err,
    );
  }
};
...
exports.registerTechnicalOfficial = async (req, res) => {
  try {
    ...
    const signatureFile = Array.isArray(files.signature)
      ? files.signature[0]
      : files.signature;
    ...
    if (official.email) {
      try {
        ...
        console.error(
          "Failed to send technical official application received email:",
          err,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "Failed to notify DDKA of technical official registration:",
        err,
      );
    }
...
exports.getAllTechnicalOfficials = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching officials" });
  }
};
...
exports.getTechnicalOfficialById = async (req, res) => {
  try {
    ...
    if (shouldExposeLoginHistory(req)) {
      payload.loginActivities = await getLoginActivities(
        official._id,
        "official",
      );
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching official" });
  }
};
...
exports.updateTechnicalOfficial = async (req, res) => {
  try {
    ...
    if (signatureFile) {
      uploadPromises.push(
        (async () => {
          ...
          if (req.params.id) {
            const existing = await TechnicalOfficial.findById(
              req.params.id,
            ).select("signatureUrl");
    ...
    const updated = await TechnicalOfficial.findByIdAndUpdate(
      req.params.id,
      updateDoc,
      {
        new: true,
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to update official" });
  }
};
  ...
  try {
    ...
    try {
      ...
      if (existing.signatureUrl)
        deletions.push(deleteCloudinaryByUrl(existing.signatureUrl));
      if (existing.photoUrl)
        deletions.push(deleteCloudinaryByUrl(existing.photoUrl));
      if (existing.receiptUrl)
        deletions.push(deleteCloudinaryByUrl(existing.receiptUrl));
      if (deletions.length) {
        await Promise.all(deletions);
      }
    } catch (err) {
      console.error(
        "Failed to delete one or more technical official assets from Cloudinary:",
        err,
      );
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to delete official" });
  }
};
...
exports.downloadOwnOfficialAsset = async (req, res) => {
  try {
    ...
    if (!req.user || req.user.role !== "official" || !req.user.id) {
      return res
        .status(401)
        .json({ success: false, message: "Official authorization required" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to download asset" });
  }
};
