  ...
  if (role === "official") {
    return {
      ...
      certificateDownloadUrl:
        "/api/technical-officials/me/certificate/download",
      createdAt: doc.createdAt,
    };
  }
  ...
  try {
    ...
    if (!type || !email || !password) {
      return res
        ...
        .json({
          success: false,
          message: "Type, email and password (registered mobile) are required",
        });
    }
    ...
    if (type === "player") {
      ...
      if (!user.idNo) {
        return res
          .status(403)
          .json({ success: false, message: "Player ID not generated yet" });
      }
      ...
      if (user.status !== "Approved") {
        return res
          ...
          .json({
            success: false,
            message: "Player registration not approved yet",
          });
      }
      ...
      if (user.status !== "Approved") {
        return res
          ...
          .json({
            success: false,
            message: "Institution registration not approved yet",
          });
      }
      ...
      if (user.status !== "Approved") {
        return res
          ...
          .json({
            success: false,
            message: "Official registration not approved yet",
          });
      }
      ...
      const donations = await Donation.find({
        email: { $regex: `^${lowerEmail}$`, $options: "i" },
      });
      ...
      const confirmedDonations = donations.filter((d) => {
        ...
        return (
          stored &&
          lastN(stored, 10) === lastN(provided, 10) &&
          d.status === "confirmed"
        );
      });
      ...
      if (!confirmedDonations || confirmedDonations.length === 0) {
        return res
          ...
          .json({
            success: false,
            message:
              "No confirmed donations found. Your donation is pending verification.",
          });
      }
      ...
      user = {
        ...
        confirmedDonations: confirmedDonations.map((d) => ({
          id: d._id,
          amount: d.amount,
        ...
        latestDonationId: confirmedDonations[0]
          ? confirmedDonations[0]._id
          : null,
      };
      role = "donor";
    } else {
    ...
    if (!user) {
      return res
        ...
        .json({
          success: false,
          message: "Invalid credentials or record not found",
        });
    }
    ...
    if (role === "donor") {
      ...
      tokenId = lowerEmail;
      try {
        extra.donorPhone = String(provided).replace(/\D/g, "").slice(-10);
      } catch (e) {
        extra.donorPhone = "";
      }
    } else {
    ...
    if (["player", "institution", "official"].includes(role)) {
      const activityUserId =
        user && (user._id || user.id) ? user._id || user.id : null;
      if (activityUserId) {
  ...
  try {
    ...
    if (role === "player") user = await Player.findById(id);
      ...
      const confirmed = donations.filter((d) => {
        ...
        return (
          d.status === "confirmed" &&
          lastN(stored, 10) === lastN(donorPhone, 10)
        );
      });
      ...
      user = {
        ...
        confirmedDonations: confirmed.map((d) => ({
          id: d._id,
          amount: d.amount,
          receiptNumber: d.receiptNumber,
          createdAt: d.createdAt,
